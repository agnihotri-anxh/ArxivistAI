...
interface SystemStatus {
  ...
  status: string;
  logs: string[];
  recent_processed_papers?: ProcessedPaperItem[];
}
...
export function AdminPanel() {
  ...
  const cancelAction = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/pipeline/cancel', { method: 'POST' });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || 'Cancel failed');
      setActionMessage('Pipeline cancellation requested.');
      fetchStatus();
    } catch (err: any) {
      setActionMessage(`Error: ${err.message}`);
    } finally {
  ...
  return (
    <div className="min-h-[calc(100vh-56px)] bg-[#f8fafc] py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
            ...
            {status?.is_running && (
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-900 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-semibold">Active Step:</span>
                  <button
                    onClick={cancelAction}
                    disabled={loading}
                    className="px-2 py-0.5 bg-rose-600 hover:bg-rose-700 text-white font-bold text-[11px] rounded-lg transition-colors flex items-center gap-1"
                  >
                    <XCircle className="w-3.5 h-3.5" />
                    Cancel Task
                  </button>
                </div>
                <p className="text-xs">{status.current_step}</p>
              </div>
            )}
        ...
        {/* Live Processed Papers Audit Table */}
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100">
            <div>
              <h3 className="font-bold text-slate-900 text-lg flex items-center gap-2">
                <Clock className="w-5 h-5 text-emerald-600" />
                Live Processed Papers Audit Log
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Real-time paper records processed by harvesting, extraction, and embedding pipeline tasks.
              </p>
            </div>
            <span className="text-xs font-semibold px-3 py-1 bg-slate-100 text-slate-600 rounded-full">
              {status?.recent_processed_papers?.length || 0} Recent Records
            </span>
          </div>
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-slate-200 text-slate-400 uppercase tracking-wider font-semibold">
                  <th className="py-3 px-3">Paper ID</th>
                  <th className="py-3 px-3">Paper Title</th>
                  <th className="py-3 px-3">Category</th>
                  <th className="py-3 px-3">Pipeline Status</th>
                  <th className="py-3 px-3 text-right">Processed Time</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 font-medium">
                {status?.recent_processed_papers && status.recent_processed_papers.length > 0 ? (
                  status.recent_processed_papers.map((item, idx) => {
                    const style = getCategoryBadgeStyle(item.category);
                    return (
                      <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3 px-3 font-mono font-bold text-slate-800">{item.paper_id}</td>
                        <td className="py-3 px-3 font-semibold text-slate-900">{item.title}</td>
                        <td className="py-3 px-3">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${style.bg} ${style.text} ${style.border}`}>
                        ...
                        <td className="py-3 px-3">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${
                            item.status === 'Milvus Indexed' 
                              ? 'bg-purple-100 text-purple-800' 
                              : item.status === 'Text Extracted' 
                                ? 'bg-violet-100 text-violet-800' 
                                : item.status === 'PDF Downloaded' 
                        ...
                        <td className="py-3 px-3 text-right font-mono text-slate-500">{item.timestamp}</td>
                      </tr>
                    );
                  })
                ) : (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-400 italic">
                      No ingestion audit records captured yet. Run a pipeline task to view live processed paper records.
                    </td>
                  </tr>
                )}
              </tbody>
